...
class ShapeCoverageTest : TestCase() {
    ...
    /** Each type descent arm casts to its bucket's type, so a shape filed in the wrong bucket must fail here. */
    fun testEveryTypeDescentBucketReceivesWhatItsArmCastsTo() {
        val miscast = GrammarShapes.CONCRETE
            .filter { !TypeDescent.classifier.classify(it).reads.isAssignableFrom(it) }
            .map { "${GrammarShapes.name(it)} in ${TypeDescent.classifier.classify(it)}" }
    ...
    /**
     * Each descent arm casts to its bucket's type, so a shape filed in the wrong bucket must fail here, not at
     * resolve.
     */
    fun testEveryDescentBucketReceivesWhatItsArmCastsTo() {
        val miscast = GrammarShapes.CONCRETE
            .filter { !VariableDescent.classifier.classify(it).reads.isAssignableFrom(it) }
            .map { "${GrammarShapes.name(it)} in ${VariableDescent.classifier.classify(it)}" }
