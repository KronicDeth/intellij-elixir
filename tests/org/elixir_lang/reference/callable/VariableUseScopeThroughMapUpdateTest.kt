 ...
 * expression, so the use scope walk must carry through the arguments to the statement that holds them, as
 * `isVariable` does.
 */
class VariableUseScopeThroughMapUpdateTest : PlatformTestCase() {
    fun testVariableBoundInsideMapUpdateHasAUseScope() {
        ...
        assertEmpty(errors)
        assertEquals(listOf("y = %{m | k: x = 1}"), (useScope as LocalSearchScope).scope.map { it.text })
    }
}
