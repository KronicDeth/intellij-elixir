...
// JPS ProcessAdapter/Key APIs are deprecated/raw; no stable replacement available yet.
...
public class ProcessAdapter extends com.intellij.execution.process.ProcessAdapter {
    ...
    private static final Pattern WARNING_PATTERN =
            Pattern.compile("^(?:\\u001B\\[[;\\d]*m)*\\s*warning:\\s*(.*)", Pattern.DOTALL);
    ...
    public void onTextAvailable(@NotNull ProcessEvent event, @NotNull Key outputType) {
        switch (state) {
            case INITIAL:
                onTextAvailableInInitial(event);
                break;
    ...
    public void processTerminated(@NotNull ProcessEvent event) {
        emitInfoMessage(builderName + " exited with code " + event.getExitCode());
    }

    private void onTextAvailableInAny(@NotNull ProcessEvent event) {
        String eventText = event.getText();

        if (!isBlank(eventText)) {
            emitInfoMessage(eventText);
        }
    }
    ...
    private void onTextAvailableInInitial(@NotNull ProcessEvent event) {
        String text = event.getText();
    ...
    private void onTextAvailableInWarning(@NotNull ProcessEvent event, @NotNull Key outputType) {
        ...
        if (text.startsWith(INDENT)) {
            String unindented = text.substring(INDENT.length());
    ...
    private void emitInfoMessage(@NotNull String messageText) {
        CompilerMessage compilerMessage = new CompilerMessage(
                builderName,
                BuildMessage.Kind.INFO,
                messageText,
                null,
                DEFAULT_PROBLEM_BEGIN_OFFSET,
                DEFAULT_PROBLEM_END_OFFSET,
                DEFAULT_PROBLEM_LOCATION_OFFSET,
                DEFAULT_LINE,
                DEFAULT_COLUMN
        );
