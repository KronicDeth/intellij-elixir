 ...
 */
public class GeneralCommandLine extends UserDataHolderBase {
    private static final Logger logger = Logger.getInstance(GeneralCommandLine.class);

    private String myExePath = null;
    private File myWorkDirectory = null;
    private final Map<String, String> myEnvParams = new MyHashMap();
    ...
    public List<String> getCommandLineList(@Nullable final String exeName) {
        ...
        String resolvedExeName = exeName != null ? exeName : Objects.requireNonNullElse(myExePath, "<null>");
        commands.add(resolvedExeName);
    ...
    public Process createProcess() throws ExecutionException {
        if (logger.isDebugEnabled()) {
            logger.debug("Executing [" + getCommandLineString() + "]");
        }
    ...
    private static class MyHashMap extends HashMap<String, String> {
        ...
        public String put(String key, String value) {
            if (key == null || value == null) {
                logger.error(new Exception("Nulls are not allowed"));
                return null;
            }
            if (key.isEmpty()) {
                // Windows: passing an environment variable with empty name causes "CreateProcess error=87, The parameter is incorrect"
                logger.warn("Skipping environment variable with empty name, value: " + value);
                return null;
            }
