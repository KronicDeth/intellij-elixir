...
class MixDepsSyncService(private val project: Project, cs: CoroutineScope) {
    ...
    internal suspend fun drain() {
        syncMutex.withLock {
            ...
            } else if (coalescedRequests.consolidatedRoots.isNotEmpty()) {
                // Only consolidated protocol libraries changed — no full dep rescan needed.
                withBackgroundProgress(project, "Syncing consolidated protocols") {
                    edtWriteAction {
                        val buildRoots = coalescedRequests.consolidatedRoots
                            .map { it.contentRootCandidate }
                            .filter { it.isValid }
                        ...
                        buildRoots.forEach { buildRoot ->
                            if (!project.isDisposed) syncConsolidatedLibrary(buildRoot)
                        }
                    }
                }
            }
    ...
    private suspend fun applySyncPlan(plan: SyncPlan) {
        edtWriteAction {
            ...
            // Sync consolidated protocol libraries outside the per-dep loop.
            val buildRoots = ProjectRootManager.getInstance(project)
                .contentRootsFromAllModules
                .mapNotNull { it.findChild("_build") }
                .filter { it.isValid }
                .distinctBy { it.path }
    ...
    // ------------------------------------------------------------------
    // Consolidated protocol library sync
    // ------------------------------------------------------------------
     ...
     */
    @VisibleForTesting
    internal fun syncConsolidatedLibrary(build: VirtualFile) {
        ...
        val library: Library = libraryTable.getLibraryByName(consolidatedLibraryName)
            ?: run {
                ...
                val created = model.createLibrary(consolidatedLibraryName, Kind)
                model.commit()
                created
            }
        ...
        // Wire it into the owner module if not already present.
        val ownerModule = build.parent?.let { ModuleUtil.findModuleForFile(it, project) } ?: return

        val alreadyAdded = ModuleRootManager.getInstance(ownerModule).orderEntries
