...
class DepsCheckerService(private val project: Project) : Disposable {
    ...
    private fun isDepsChange(event: VFileEvent): Boolean {
        ...
        return roots.any { root ->
            ...
            val depsPath = "$rootPath/deps"
            val buildPath = "$rootPath/_build"
            val mixExsPath = "$rootPath/mix.exs"
            val mixLockPath = "$rootPath/mix.lock"

            FileUtil.isAncestor(depsPath, eventPath, false) ||
                    FileUtil.isAncestor(buildPath, eventPath, false) ||
                    FileUtil.pathsEqual(eventPath, mixExsPath) ||
                    FileUtil.pathsEqual(eventPath, mixLockPath)
        }
    }
