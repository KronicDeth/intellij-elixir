...
abstract class Type : PsiScopeProcessor {
    ...
    private fun execute(call: Call, state: ResolveState): Boolean =
        when (call) {
            ...
            is AtUnqualifiedNoParenthesesCall<*> -> execute(call, state)
            else -> if (call.semantic is Modular && call.isAncestor(state.get(ENTRANCE), false)) {
                ...
                // specialization
                if (childCallsKeepProcessing && call.semantic is Protocol) {
            ...
            } else if (call.semantic is org.elixir_lang.semantic.Use) {
    ...
    private fun executeOnDecompiledProtocol(call: Call): Boolean {
        ...
        return call.name?.let { name ->
            StubIndex.getInstance().processElements(
                ...
                if (modular is ModuleDefinitionImpl) {
                    ...
                    execute(
                        decompiled,
                        ResolveState.initial().put(ENTRANCE, decompiled).putInitialVisitedElement(decompiled)
                    )
                } else {
                    true
                }
