...
abstract class Variable : PsiScopeProcessor {
    ...
    private fun isInDeclaringScope(call: Call, state: ResolveState): Boolean =
            ...
            ?: PsiTreeUtil
                ...
                ?.let { maybeDeclaringScopeContext ->
                    if (maybeDeclaringScopeContext is ElixirStabOperation) {
                        val signature: PsiElement? = maybeDeclaringScopeContext.leftOperand()
                        if (PsiTreeUtil.isAncestor(signature, call, false)) {
                            isDeclaringScope(maybeDeclaringScopeContext)
                        } else {
                            false
                        }
                    } else if (maybeDeclaringScopeContext is InMatch) {
                        PsiTreeUtil.isAncestor(maybeDeclaringScopeContext.leftOperand(), call, false)
                    } else {
                        false
                    }
                }
    ...
    private fun execute(semantic: Semantic, state: ResolveState): Boolean =
        when (semantic) {
            is org.elixir_lang.semantic.call.definition.clause.Call ->
                semantic.head?.let { head ->
                    when (val stripped = strip(head.psiElement)) {
                        is AtOperation -> {
                            stripped
                                .operand()
            ...
            is org.elixir_lang.semantic.call.definition.Delegation ->
                whileIn(semantic.head.parameters) {
                    execute(it, state)
                }
            is org.elixir_lang.semantic.Destructure -> {
                semantic.output?.let { execute(it, state.put(DECLARING_SCOPE, true)) }
            }
            is org.elixir_lang.semantic.Chain -> {
                val entrance = state.get(ElixirPsiImplUtil.ENTRANCE)
                val clauses = semantic.clauses

                /* if the entrance isn't in the clauses, then it is part of the block and so search should start
                   from the last argument */
                var entranceClauseIndex = clauses.size - 1

                // have to check in reverse order as a variable can be rebound in the `<-` and `=` of a `for` or `with`
                for (i in clauses.indices.reversed()) {
                    if (PsiTreeUtil.isAncestor(clauses[i].psiElement, entrance, false)) {
                        entranceClauseIndex = i
                        break
                    }
                }
                ...
                for (i in entranceClauseIndex downTo 0) {
                    ...
                    keepProcessing = execute(clause, state.put(DECLARING_SCOPE, false))
                    if (!keepProcessing) {
                        break
                    }
                }
            ...
            is org.elixir_lang.semantic.MatchQuestionMark -> {
                val declaringScope = when (val patternPsiElement = semantic.pattern.psiElement) {
                    is When -> patternPsiElement.leftOperand()
                    else -> patternPsiElement
                }

                declaringScope?.semantic?.let {
                    execute(it, state.put(DECLARING_SCOPE, true))
                }
            }
            is org.elixir_lang.semantic.quote.Call ->
                whileIn(semantic.quoteBoundVariables) {
                    execute(it, state)
                }
                        &&
                        semantic.declaredVariable?.let { execute(it, state) } ?: true
    ...
    private fun execute(match: Call, state: ResolveState): Boolean {
        val semantic = match.semantic

        return when {
            semantic != null -> execute(semantic, state)
                ...
                if (!match.isCalling(Module.KERNEL, Function.UNQUOTE, 1)) {
                    ...
                    when {
                        ...
                        maybeMacro(match, state) -> {
                            /* macro uses in stab signatures see
