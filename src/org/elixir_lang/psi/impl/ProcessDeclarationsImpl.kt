        ...
        if (!call.isEquivalentTo(lastParent) || call.isEquivalentTo(place)) {
            when {
                ...
                /* Any other call's arguments are values, so what they hold is read, but a match inside one binds a
                   ...
                   before its own, and a call met as an ancestor is not walked past that point. Only the variable
                   resolver reads arguments; a type or module walk finds nothing in a value. */
                processor is Variable -> call.finalArguments()?.let { arguments ->
                    ...
                    whileIn(before) { processor.execute(it, reading) }
                } ?: true
                else -> true
            }
        } else {
    ...
    fun processDeclarations(
        ...
        sequence
            ...
            .map {
                ...
                if (it is Call || processor !is Variable) {
