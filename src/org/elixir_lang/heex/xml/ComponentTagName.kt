...
/**
 * Splits a HEEx tag name into the component or slot it refers to, mirroring
 ...
 * are the module alias chain, exactly as written in the source); a name starting with `:` is a
 * slot (see [HeexSlotDescriptor]). Any other name - including a bare uppercase name with no dot,
 * which Phoenix itself fails to compile - is not ours and this returns `null`.
 */
sealed class ComponentTagName {
    ...
    data class Slot(val slotName: String) : ComponentTagName()
    ...
    companion object {
        ...
        fun parse(tagName: String): ComponentTagName? = when {
            ...
            tagName.startsWith(":") ->
                tagName.removePrefix(":").takeIf { it.isNotEmpty() }?.let(::Slot)
