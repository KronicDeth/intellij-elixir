    ...
    override fun walk(call: Call,
            ...
            when (resolvedFinalArity) {
                ...
                2 ->
                    ...
                    Query.executeOnSelectExpression(arguments[arguments.lastIndex], state.put(Query.CALL, call), keepProcessing)
...
private object Dynamic: NameArityRangeWalker("dynamic", 1..2) {
    override fun walk(call: Call, arguments: Array<PsiElement>, resolvedFinalArity: Arity, state: ResolveState, keepProcessing: (element: PsiElement, state: ResolveState) -> Boolean): Boolean =
            ...
            when (resolvedFinalArity) {
                // `dynamic(expr)`
                1 -> Query.executeOnSelectExpression(arguments[arguments.lastIndex], state.put(Query.CALL, call), keepProcessing)
                // `dynamic(binding, expr)
            ...
            when (fromKeywords) {
                ...
                is QuotableKeywordPair -> {
                    when (val keywordKeyText = fromKeywords.keywordKey.text) {
                        ...
                        "left_lateral_join", "right_join" -> Query.executeOnIn(fromKeywords.keywordValue, state, keepProcessing)
...
object Query: ModuleWalker("Ecto.Query", Distinct, Dynamic, From, GroupBy, Having, Join, Lock, OrderBy, Preload, Select, SelectMerge, Update, Where, Windows, WithCTE) {
    ...
    internal fun executeOnHavingOrOnOrWhere(element: PsiElement,
                                            state: ResolveState,
                                            keepProcessing: (element: PsiElement, state: ResolveState) -> Boolean): Boolean =
            when (element) {
                is Call -> keepProcessing(element, state)
