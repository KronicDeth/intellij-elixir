...
class TabbedPane(lines: Lines?): JTabbedPane(JTabbedPane.TOP, JTabbedPane.WRAP_TAB_LAYOUT) {
    init {
        addTab("Line References", JBScrollPane(Table(LineReferencesModel(lines?.lineReferenceList.orEmpty()))))
        addTab("File Names", JBScrollPane(FileNamesTable(FileNamesModel(lines?.fileNameList.orEmpty()))))
    }
}
