...
public class Elixir extends JpsSdkType<SdkProperties> implements JpsElementTypeWithDefaultProperties<SdkProperties> {
    ...
    private static String mixHome(@Nullable String maybeHomePath) throws MissingHomePath {
        ...
        if (source == null || !HomePath.VERSION_MANAGERS.contains(source)) {
            return null;
        }

        File mixFile = mixFile(homePath);
        File binDir = mixFile.getParentFile();
        if (binDir == null) {
            return null;  // Invalid path structure
        }

        String parentPath = binDir.getParent();
        if (parentPath == null) {
            return null;  // No parent directory (root or invalid path)
        }

        // MIX_HOME = $(dirname mixPath)/../.mix
        return new File(parentPath, ".mix").getAbsolutePath();
    }
